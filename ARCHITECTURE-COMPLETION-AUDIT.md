...
#### 4. Add the socket event stream and route the TUI through projections

The Unix socket is request/response only. There is no protocol command for event subscription, replay from a cursor, subscriber acknowledgement, or slow-consumer recovery.
...
- Add versioned `SubscribeEvents`, `ResumeEvents`, and cursor/ack envelopes.
- Bound subscriber buffers and define explicit lag recovery.
- Add a TUI client connection with reconnect and replay.
...
The protocol has an `ExecutionMode` and operation capability flags, but it does not define or implement the requested first-class models:

- `InteractiveSession`
- `DeferredTask`
- `DetachedJob`

Long-running jobs need durable accepted receipts, attachment/detachment, cursor continuation, reattachment, and mode-specific cancellation and recovery semantics. Existing background-job machinery is useful but is not integrated into the new tasking protocol.
...
#### 12. Add typed runtime profiles and an offline profile linter

The Cobalt Strike-derived recommendation called for declarative, versioned profiles validated before activation. No control-plane runtime-profile type, activation transaction, or offline linter is present.

Profiles should cover model artifacts, adapters, provider compatibility, concurrency, resource limits, spool policy, and required capability hashes.
...
| Interactive/deferred/detached split | Capability flags only | Not complete |
...
| Read projections | Basic projections only; TUI integration absent | Committed in `c5e60e1` |
