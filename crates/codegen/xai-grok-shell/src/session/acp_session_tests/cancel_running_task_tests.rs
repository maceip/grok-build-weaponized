...
fn persist_ack_waits_for_disk_flush_before_success() {
    std::thread::Builder::new()
        .name("persist-ack-test".to_string())
        .stack_size(32 * 1024 * 1024)
        .spawn(|| {
            let runtime = tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()
                .expect("persist-ack runtime");
                ...
                local
                    .run_until(async {
                        ...
                        let actor = Arc::new(SessionActor {
                            ...
                            chat_state_handle,
                            unattributed_background_usage: std::sync::atomic::AtomicBool::new(
                                false,
                            ),
                            ...
                            last_reported_branch: std::sync::Arc::new(parking_lot::Mutex::new(
                                None,
                            )),
                            ...
                            workflow_manager:
                                crate::session::workflow::manager::WorkflowManager::test_bundle().0,
                            ...
                            tool_metadata_snapshot: Arc::new(std::sync::Mutex::new(
                                Default::default(),
                            )),
                            ...
                            extension_registry:
                                xai_agent_lifecycle::LocalExtensionRegistry::default(),
                        ...
                        let storage =
                            crate::session::storage::JsonlStorageAdapter::with_explicit_session_dir(
                                session_dir,
                            );
                        let loaded = storage
        ...
        .expect("persist-ack test thread")
        .join()
        .expect("persist-ack test thread panicked");
}
